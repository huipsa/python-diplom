...
from my_app.models import Contact, Order, Shop
...
def test_login_account_success():
    ...
    user = User.objects.create_user(email="johndoe@example.com", password="StrongPassword123!")
...
def test_contact_view_get_authenticated():
    ...
    Contact.objects.create(user=user, city="City", street="Street", phone="1234567890")
...
def test_shop_create_success():
    ...
    # Создаем пользователя
    user = User.objects.create_user(email="johndoe@example.com", password="StrongPassword123!", is_active=True)
    client.force_authenticate(user=user)
